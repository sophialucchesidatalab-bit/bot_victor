...
# Link das marinadas fixo como fallback caso a variavel de ambiente nao esteja definida
_LINK_MARINADAS = LINK_MARINADAS or "https://nutrivictorafonso.hotmart.host/marinadas-do-nutri-victor-0fd4c29e-6213-49be-be5c-32bffa06db54"
...
def get_marinadas() -> str:
    """Retorna mensagem das marinadas com link sempre atualizado."""
        link = os.getenv("LINK_MARINADAS", _LINK_MARINADAS)
            return (
                    f"Que ótima escolha! 🥩🔥\n\n"
                            f"As *Marinadas do Nutri* são temperos naturais desenvolvidos pelo próprio Nutri Victor, "
                                    f"pensados para deixar sua alimentação mais saborosa e saudável ao mesmo tempo.\n\n"
                                            f"Clica no link abaixo para conhecer e garantir o seu:\n"
                                                    f"👉 {link}\n\n"
                                                            f"Qualquer dúvida é só chamar, nossa atendente está por aqui! 💚"
                                                                )


                                                                # Mantém compatibilidade com código que usa MARINADAS diretamente
                                                                MARINADAS = get_marinadas()
                                                                ...
                                                                def notificacao_victor(paciente_nome: str, paciente_phone: str,
                                                                                        local: str = "", turno: str = "",
                                                                                                                assunto: str = "", tipo: str = "agendamento") -> str:
                                                                                                                    if tipo == "agendamento":
                                                                                                                            turno_texto = turno if turno else "Não informado"
                                                                                                                                    local_texto = local if local else "Não informado"
                                                                                                                                            return (
            f"📅 *Nova solicitação de consulta!*\n\n"
                        f"*Paciente:* {paciente_nome}\n"
                                    f"*Telefone:* {paciente_phone}\n"
                                                f"*Local:* {local_texto}\n"
                                                            f"*Turno preferido:* {turno_texto}\n\n"
